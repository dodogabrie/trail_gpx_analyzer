#!/usr/bin/env python3
"""Recover missing activities from athletes with failed downloads.
...
to download them with proper validation (elevation > 200m, latlng + altitude).

Usage:
    # Recover all missing activities
    python recover_activities.py

    # Dry run (show what would be downloaded)
    python recover_activities.py --dry-run

    # Specific athlete
    python recover_activities.py --athlete-id 11943431

    # Custom data directory
    python recover_activities.py --data-dir data/strava/athletes/
"""
...
from src.activity_downloader import ActivityDownloader
...
def recover_activities(missing_by_athlete: Dict[int, List[int]],
    ...
    try:
        ...
        if not scraper.load_session():
            ...
            print("Please run scrape_athlete.py with --chrome-profile to create session first")
            return

        downloader = ActivityDownloader(scraper)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Recover missing activities from athlete directories",
...
Examples:
  ...
  python recover_activities.py --data-dir data/strava/athletes/
    ...
    # Recover activities
    recover_activities(missing_by_athlete, data_dir, args.dry_run)
